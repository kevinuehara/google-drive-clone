...
describe("#UploadHandler test suite", () => {
  ...
  describe("#onFile", () => {
    test("given a stream file it should save it on disk", async () => {
      ...
      const expectedFile = resolve(handler.downloadsFolder, params.filename);
      expect(fs.createWriteStream).toHaveBeenCalledWith(expectedFile);
    });
  });
  ...
  describe("#handleFileBytes", () => {
    ...
    test("given message timerDelay as 2secs it should emit only two messages 3 seconds period", async () => {
        jest.spyOn(ioObj, ioObj.emit.name)
